...
export default function LaptopAssetsPage() {
  ...
  useEffect(() => {
    ...
    const checkPermissions = async () => {
      ...
      try {
        ...
        logger.debug('=== LAPTOP ASSETS PERMISSION CHECKING STARTED ===');
        ...
        logger.debug('=== LAPTOP ASSETS PERMISSION CHECKING FINISHED ===');
      }
    };
  ...
  }, [canViewLaptop, canCreateLaptop, canEditLaptop, canDeleteLaptop, canBulkDeleteLaptop, currentUser, isUserLoading]);

  // Get asset types configuration
  const assetTypes = getAssetTypes(t);
  const laptopAssetType = assetTypes.find(type => type.key === "laptop");
  ...
  return (
    <Suspense fallback={<AssetListSkeleton />}>
      <AssetList
        key="laptop" // Add key prop to ensure proper re-rendering when switching asset types
        assetType="laptop"
        title={t('assets.laptop.title') || "Laptop"}
        columns={laptopAssetType.columns}
        formFields={laptopAssetType.formFields}
        canView={canView}
        canCreate={canCreate}
        canEdit={canEdit}
        canDelete={canDelete}
